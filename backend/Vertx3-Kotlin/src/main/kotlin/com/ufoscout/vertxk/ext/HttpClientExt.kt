...
interface HttpClientExt {
    ...
    suspend fun <T : Any> HttpClient.restHead(port: Int, host: String, requestUri: String, responseClass: KClass<T>, vararg headers: Pair<String, String>): HttpClientKResponse<T> {
        return awaitEvent<HttpClientKResponse<T>> {
            val request = head(port, host, requestUri) { response -> HttpClientHelper.handlerRestResponse(response, responseClass, it) }
            HttpClientHelper.restRequest(request, it, *headers)
        }
    }
